...
public class AlertDao {
  ...
  private static final String ALERT_SHAPE_QUERY =
    "SELECT row_to_json(fc) as geojson " +
      ...
      "                 row_to_json((SELECT l FROM (SELECT area, createdTime, name, span, status, description) as l)) as properties" +
      "             FROM alert " +
  ...
  public void updateAlertOrder(List<Integer> idAlerts, Integer idOrder) {
    String idList = QueriesBuilder.listConditionValue(idAlerts);
    String query = String.format("UPDATE alert set idorder = %d WHERE id in (%s)",
      idOrder, idList);
    jdbcTemplate.execute(query);
  }

  public List<Alert> update(List<Integer> idAlerts, String description, String status) {
    String idList = QueriesBuilder.listConditionValue(idAlerts);
    String query = String.format("UPDATE alert set status = '%s', description = '%s' WHERE id in (%s)",
      status, description, idList);
    jdbcTemplate.execute(query);
    return find(idAlerts);
  }
