...
import {MarkAlertsData} from '../mark-alerts-dialog/mark-alerts-data';
...
export class AlertListComponent implements OnInit {
  ...
  private getActions(): ActionButton<Alert>[] {
    return [
      ...
      new ActionButton({
        label: 'Oznacz alarm',
        buttonType: 'flat',
        action: (alerts: Alert[]) => this.openMarkAlertDialog(alerts)
      }),
      new ActionButton({
  ...
  private openMarkAlertDialog(alerts: Alert[]): void {
    this.dialog.open(MarkAlertsDialogComponent, {
      width: '544px',
      data: new MarkAlertsData({alerts, action: () => this.search.search() })
    });
  }
